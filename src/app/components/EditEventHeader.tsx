...
const EditEventHeader = ({ eventId }: { eventId: string }) => {
  ...
  function DeleteEvent({
    ...
    return (
      <>
        <button
          className={`${styles.headerDeleteButton} ${styles.headerButton}`}
          onClick={onOpen}
        >
          Delete Event
        </button>
        <Modal isOpen={isOpen} onClose={onClose}>
          ...
          <ModalContent className={styles.confirmDeletion} maxW="fit-content">
            <ModalHeader style={{ width: '100%', textAlign: 'center' }}>
              Please confirm deletion of {eventName} Event.
            </ModalHeader>
            <ModalCloseButton />
            <ModalBody className={styles.cancelOrDelete}>
              <button
                className={`${styles.cancelButton} ${styles.confirmationButton}`}
                onClick={onClose}
              >
                Cancel
              </button>
  ...
  return (
    <Box className={styles.header}>
      <button
        className={`${styles.backButton} ${styles.headerButton}`}
        onClick={() => router.back()}
      >
        Back
      </button>
      <Box className={styles.rightButtons}>
        {registrationOpen && (
          <button
            className={`${styles.headerStopButton} ${styles.headerButton}`}
        ...
        {!registrationOpen && (
          <button
            className={`${styles.headerOpenButton} ${styles.headerButton}`}
            onClick={changeRegistrationStatus}
          >
            Open Registration
          </button>
        ...
        <DeleteEvent
          eventName={eventData?.eventName || 'Error.'}
